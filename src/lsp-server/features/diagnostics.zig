...
pub fn generateDiagnostics(server: *Server, arena: std.mem.Allocator, handle: *DocumentStore.Handle, err_bundle: ?ErrorBundle) error{OutOfMemory}!types.PublishDiagnosticsParams {
    ...
    if (tree.errors.len == 0 and tree.mode == .zig) {
        ...
        for (diagnostics_set.values()) |diag_item| try diagnostics.appendSlice(arena, diag_item.items);
...
pub fn errorBundleToDiagnostics(
    ...
    for (error_bundle.getMessages()) |msg_index| {
        ...
        //     if (item.range.start.line == src_range.start.line and
        //         item.range.start.character == src_range.start.character and
        //         item.range.end.line == src_range.end.line and
        //         item.range.end.character == src_range.end.character and
        //         std.mem.eql(
        //         u8,
        //         item.message,
        //         error_bundle.nullTerminatedString(err.msg),
        //     )) continue :msg;
        // }
        ...
        // std.debug.print("new diag: {s}: {s}\n", .{ src_path, error_bundle.nullTerminatedString(err.msg) });
    }
}

...
fn fullPtrTypeComponents(tree: *const Ast, info: full.PtrType.Components) full.PtrType {
    const size: std.lang.Type.Pointer.Size = switch (tree.tokenTag(info.main_token)) {
        .asterisk => switch (tree.tokenTag(info.main_token + 1)) {
    ...
    pub fn init(tree: *const Ast, node: Ast.Node.Index) Iterator {
        return switch (tree.nodeTag(node)) {
            ...
            .assign_destructure => {
                ...
                const sub_range_start: Ast.ExtraIndex = @fromBackingInt(@intCast(@backingInt(extra_index) + 1));
                const sub_range_end: Ast.ExtraIndex = @fromBackingInt(@intCast(@backingInt(sub_range_start) + variable_count));
                return .{ .sub_range = .{
            ...
            => {
                ...
                const else_expr: Node.OptionalIndex = if (extra.has_else) @fromBackingInt(@intCast(tree.extra_data[@backingInt(extra_index) + extra.inputs + 1])) else .none;
                return .{ .sub_range = .{
                    .items = .{ .start = extra_index, .end = @fromBackingInt(@intCast(@backingInt(extra_index) + extra.inputs)) },
    ...
    pub fn next(it: *Iterator, tree: *const Ast) ?Ast.Node.Index {
        sw: switch (it.*) {
            ...
            .sub_range => |*sub_range| {
                ...
                if (items.len > 0) {
                    defer sub_range.items.start = @fromBackingInt(@intCast(@backingInt(sub_range.items.start) + 1));
                    return items[0];
                }

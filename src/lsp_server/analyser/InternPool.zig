...
pub const Index = enum(u32) {
    ...
    pub const Slice = struct {
        ...
        pub fn at(slice: Slice, index: u32, ip: *InternPool) Index {
            ...
            return @fromBackingInt(@intCast(ip.extra.items[slice.start + index]));
        }
...
fn indexToKeyNoLock(ip: *const InternPool, index: Index) Key {
    ...
    return switch (item.tag) {
        .simple_type => .{ .simple_type = @fromBackingInt(@intCast(data)) },
        .simple_value => .{ .simple_value = @fromBackingInt(@intCast(data)) },
        ...
        .type_optional => .{ .optional_type = .{ .payload_type = @fromBackingInt(@intCast(data)) } },
        .type_anyframe => .{ .anyframe_type = .{ .child = @fromBackingInt(@intCast(data)) } },
        ...
        .type_struct => .{ .struct_type = @fromBackingInt(@intCast(data)) },
        .type_enum => .{ .enum_type = @fromBackingInt(@intCast(data)) },
        .type_union => .{ .union_type = @fromBackingInt(@intCast(data)) },
        ...
        .null_value => .{ .null_value = .{ .ty = @fromBackingInt(@intCast(data)) } },
        .undefined_value => .{ .undefined_value = .{ .ty = @fromBackingInt(@intCast(data)) } },
        .unknown_value => .{ .unknown_value = .{ .ty = @fromBackingInt(@intCast(data)) } },
    };
}
...
pub fn zigTypeTag(ip: *InternPool, index: Index) ?std.lang.TypeId {
    ...
    return switch (ip.items.items(.tag)[@backingInt(index)]) {
        .simple_type => switch (@as(SimpleType, @fromBackingInt(@intCast(ip.items.items(.data)[@backingInt(index)])))) {
            .f16,
            .f32,
            .f64,
            .f80,
...
pub fn typeOf(ip: *InternPool, index: Index) Index {
    ...
    return switch (ip.items.items(.tag)[@backingInt(index)]) {
        .simple_value => switch (@as(SimpleValue, @fromBackingInt(@intCast(data)))) {
        ...
        .union_value,
        .error_value,
        => @fromBackingInt(@intCast(ip.extra.items[ip.items.items(.data)[@backingInt(index)]])),

        // the type is the `data` field
        .null_value,
        .undefined_value,
        .unknown_value,
        => @fromBackingInt(@intCast(ip.items.items(.data)[@backingInt(index)])),
    };
}
...
pub fn isType(ip: *InternPool, ty: Index) bool {
    ...
    return switch (ip.items.items(.tag)[@backingInt(ty)]) {
        ...
        .undefined_value,
        => false,
        .unknown_value => .unknown_type == @as(Index, @fromBackingInt(@intCast(ip.items.items(.data)[@backingInt(ty)]))),
    };
}

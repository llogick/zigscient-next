...
pub fn generateDiagnostics(
    ...
    return generateOptionalDiagnostics(server, handle);
}

pub fn generateOptionalDiagnostics(
    server: *Server,
    handle: *DocumentStore.Handle,
) Analyser.Error!void {
...
pub const BuildOnSave = struct {
    ...
    fn handleWatchErrorBundle(
        ...
        try collection.collectNotVisibleErrMessages(ds, diagnostic_tag, header.cycle, workspace_path, eb);

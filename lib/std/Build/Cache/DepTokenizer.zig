        ...
        switch (self.state) {
            ...
            .target_dollar_sign => switch (char) {
                '$' => {
                    ...
                    return errorIllegalChar(.expected_dollar, self.index, char);
                },
            },
...
pub const Token = union(enum) {
    ...
    pub fn format(token: Token, w: *Io.Writer) Io.Writer.Error!void {
        switch (token) {
            .target, .target_must_resolve, .prereq, .prereq_must_resolve => return w.writeAll(@tagName(token)),
            .incomplete_quoted_prerequisite, .incomplete_target => |info| return w.print("{t} {q} at position {d}", .{
                token, info.bytes, info.index,
            }),
            .invalid_target,
            .bad_target_escape,
            .expected_dollar,
            ...
            .incomplete_escape,
            .expected_colon,
            => |info| return w.print("illegal char '{f}' at position {d}: {t}", .{
                std.zig.fmtChar(info.char), info.index, token,
            }),
        }
    }
};
...
test "error incomplete escape - reverse_solidus" {
    try depTokenizer("\\",
        \\ERROR: illegal char '\\' at position 0: incomplete_escape
    );
    try depTokenizer("\t\\",
        \\ERROR: illegal char '\\' at position 1: incomplete_escape
    );
    try depTokenizer("\n\\",
        \\ERROR: illegal char '\\' at position 1: incomplete_escape
    );
    ...
    try depTokenizer("\r\n\\",
        \\ERROR: illegal char '\\' at position 2: incomplete_escape
    );
    try depTokenizer(" \\",
        \\ERROR: illegal char '\\' at position 1: incomplete_escape
    );
...
test "error incomplete target" {
    try depTokenizer("foo.o",
        \\ERROR: incomplete_target "foo.o" at position 0
    );
    try depTokenizer("\tfoo.o",
        \\ERROR: incomplete_target "foo.o" at position 1
    );
    try depTokenizer("\nfoo.o",
        \\ERROR: incomplete_target "foo.o" at position 1
    );
    ...
    try depTokenizer("\r\nfoo.o",
        \\ERROR: incomplete_target "foo.o" at position 2
    );
    try depTokenizer(" foo.o",
        \\ERROR: incomplete_target "foo.o" at position 1
    );
...
test "error illegal char at position - bad target escape" {
    try depTokenizer("\\\t",
        \\ERROR: illegal char '\t' at position 1: bad_target_escape
    );
    try depTokenizer("\\\n",
        \\ERROR: illegal char '\n' at position 1: bad_target_escape
    );
    try depTokenizer("\\\r",
        \\ERROR: illegal char '\r' at position 1: bad_target_escape
    );
    try depTokenizer("\\\r\n",
        \\ERROR: illegal char '\r' at position 1: bad_target_escape
    );
...
test "error illegal char at position - expecting dollar_sign" {
    try depTokenizer("$\t",
        \\ERROR: illegal char '\t' at position 1: expected_dollar
    );
    try depTokenizer("$\n",
        \\ERROR: illegal char '\n' at position 1: expected_dollar
    );
    try depTokenizer("$\r",
        \\ERROR: illegal char '\r' at position 1: expected_dollar
    );
    try depTokenizer("$\r\n",
        \\ERROR: illegal char '\r' at position 1: expected_dollar
    );
...
test "error illegal char at position - invalid target" {
    try depTokenizer("foo\n.o",
        \\ERROR: illegal char '\n' at position 3: invalid_target
    );
    try depTokenizer("foo\r.o",
        \\ERROR: illegal char '\r' at position 3: invalid_target
    );
    try depTokenizer("foo\r\n.o",
        \\ERROR: illegal char '\r' at position 3: invalid_target
    );
...
test "error target - continuation expecting end-of-line" {
    try depTokenizer("foo.o: \\\t",
        \\target = {"foo.o"}
        \\ERROR: illegal char '\t' at position 8: continuation_eol
    );
    try depTokenizer("foo.o: \\ ",
        \\target = {"foo.o"}
        \\ERROR: illegal char ' ' at position 8: continuation_eol
    ...
    try depTokenizer("foo.o: \\x",
        \\target = {"foo.o"}
        \\ERROR: illegal char 'x' at position 8: continuation_eol
    );
    try depTokenizer("foo.o: \\\x0dx",
        \\target = {"foo.o"}
        \\ERROR: illegal char 'x' at position 9: continuation_eol
    );
}
...
test "error prereq - continuation expecting end-of-line" {
    try depTokenizer("foo.o: foo.h\\\x0dx",
        \\target = {"foo.o"}
        \\ERROR: illegal char 'x' at position 14: continuation_eol
    );
}
...
test "error illegal char at position - expecting colon" {
    try depTokenizer("foo\t.o:",
        \\target = {"foo"}
        \\ERROR: illegal char '.' at position 4: expected_colon
    );
    try depTokenizer("foo .o:",
        \\target = {"foo"}
        \\ERROR: illegal char '.' at position 4: expected_colon
    ...
    try depTokenizer("foo \n.o:",
        \\target = {"foo"}
        \\ERROR: illegal char '\n' at position 4: expected_colon
    );
    try depTokenizer("foo.o\t\n:",
        \\target = {"foo.o"}
        \\ERROR: illegal char '\n' at position 6: expected_colon
    );
}
...
fn depTokenizer(input: []const u8, expect: []const u8) !void {
    ...
    while (it.next()) |token| {
        ...
        switch (token) {
            .target, .prereq => |bytes| {
                try buffer.print(arena, "{t} = {{{q}}}", .{ token, bytes });
            },
            .target_must_resolve => {
                try buffer.appendSlice(arena, "target = {");
                aux.clearRetainingCapacity();
                try token.resolve(arena, &aux);
                try buffer.print(arena, "{q}", .{aux.items});
            ...
            .prereq_must_resolve => {
                ...
                try buffer.print(arena, "ERROR: {f}", .{token});
                break;
            },
        }
        i += 1;
